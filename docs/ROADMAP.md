...
[████████████████████] 100%  Phase 5 ✅ 完成 (20+ 功能: 技能/觅食/季节/繁衍/徽章..., 剩多模态→Phase 13)
...
- [ ] ① 数码兽 TTS 配音 — 每只数码兽有独特声线 (Edge TTS / Fish Audio)
- [ ] ② 进化/战斗动画增强 — sprite sheet 动画替换当前 CSS 动画
- [x] ③ 后端性能优化 — DB 索引(6个) + WAL mode + mmap + /api/health/perf 端点 ✅ (`fb5d350`)
- [ ] ④ 前端性能优化 — PixiJS 纹理批处理、虚拟列表
- [ ] ⑤ 世界存档系统 — 定时快照 + 版本迁移 + 回滚 ✅
- [ ] ⑥ 管理后台 — 世界管理/Agent 控制/日志查询 Web UI
- [~] ⑦ 自动化部署增强 — CI (.github/workflows/ci.yml) + 健康检查 (scripts/health_check.sh) ✅, 告警/部署增强待完成
- [ ] ⑧ Phase 5 多模态迁移 — 从 Phase 5 待办中迁移完成
