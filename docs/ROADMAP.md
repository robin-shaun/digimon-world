...
|
|- [ ] Task 1 — `memory_autonomy.py` 核心模块: LLM 自评重要性 + Ebbinghaus 遗忘曲线 + 记忆复述 + 过期检测
|- [ ] Task 2 — 接入 DigimonAgent 主循环: agent.step() 中调用记忆自主评估
|- [ ] Task 3 — API 端点: `/api/digimon/{name}/memory-health` 记忆健康诊断
|- [ ] Task 4 — 前端记忆健康面板: 遗忘曲线可视化 + 过期记忆列表
|- [ ] Task 5 — 集成测试 + 端到端验证
|
