...
## Phase 15: 导演面板 2.0 — 事件注入 + 世界干预 🔜

**目标**: 用户可通过导演面板向世界注入自定义事件，实时干预世界走向。

- [ ] Task 1 — 后端事件注入 API — POST /api/director/inject-event + 参数校验
- [ ] Task 2 — 事件模板系统 — 预设事件类型（天灾/节日/战斗/剧情/进化）
- [ ] Task 3 — 前端注入面板 UI — 事件类型选择 + 参数表单 + 预览 + 发送
- [ ] Task 4 — 注入事件生效 — Scheduler 处理注入事件 + 世界状态变化验证
- [ ] Task 5 — 事件历史面板 — 已注入事件列表 + 结果反馈

**完成标志**: 用户可随时注入自定义事件，世界立即响应。