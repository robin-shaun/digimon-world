...
## Phase 15: 前端体验增强 — 注入反馈 + 事件日志 🔜

**目标**: 事件注入后能看到实时反馈和影响范围，导演面板增加事件历史。

- [ ] Task 1 — 后端注入反馈增强 — /api/director/inject_event 返回 affected_agents + impact_summary
- [ ] Task 2 — 前端事件历史面板 — 导演面板中展示最近 10 条注入事件 + 展开详情
- [ ] Task 3 — 注入结果实时展示 — 注入后高亮受影响数码兽 + 弹出通知
- [ ] Task 4 — 事件模板扩展 — 新增"进化事件""节日事件""遭遇事件"等预设模板
