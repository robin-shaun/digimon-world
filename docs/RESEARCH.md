...
#### 1. 【arXiv 2607.15095】Digital Pantheon: Simulating and Auditing Coalition Formation with LLM Agents
> 多智能体政治联盟模拟 — SFT + DPO + RAG 驱动党派人格

**核心**: 用 DPO (Direct Preference Optimization) 给 LLM agent 注入"坚定党派人格"——RLHF 训练出的 neutrality bias 会让 agent 无法维持立场，DPO 解决了这个问题。同时用 RAG 管线约束 agent 在官方宣言范围内行动。引入 **MILT (Multi-Layered Information Lineage Topology)** 追踪最终协议中每条条款的来源，以及 **CIS (Coalition Influence Score)** 量化各方影响力。

**对数码宝贝世界的影响** ⭐⭐⭐⭐:
- 我们的派系系统（Phase 4）和差序格局（Phase 16）已有关系追踪，但缺乏 **DPO 式的"坚定人格"机制**——数码兽应该在社交中保持自己的性格，而不是被 LLM 的 neutrality 漂移
- **建议**: Phase 17 的 MBTI 人格系统可以借鉴 DPO 思路——不是让 LLM 扮演 INFP，而是用结构化约束（偏好向量 + 行为掩码）确保人格一致性
- MILT 追踪决策来源的思路可以直接用于我们的**因果链日志**增强（Phase 7 已有基础）

#### 2. 【GitHub】mirage-cortex — Top Agent Swarm Simulation Tools for LLM World Modeling 2026
> ⭐152 | 智能体群模拟工具精选列表

**核心**: 一份精心整理的 2026 年 LLM 世界建模工具清单，涵盖 agent swarm 模拟框架。

**对数码宝贝世界的影响** ⭐⭐:
- 竞品格局参考：目前 agent swarm 赛道还在工具阶段，多数项目 <50 agent、<1天运行
- 我们的 30 agent × 持续运行 + RPG 战斗/进化系统，在这个赛道上差异化明显

#### 3. 【GitHub 竞品】GoldenHouse-Writer / novel-studio / SoulSim — 中文 AI 世界模拟引擎涌现
> GoldenHouse-Writer: 多智能体 AI 小说创作 & 世界模拟引擎
> novel-studio (⭐11): 全自动 AI 长篇小说创作引擎 — 动态世界推演 · 多智能体长循环 · 本地 RAG
> SoulSim (⭐5): 多智能体世界模拟引擎 — 自动生成世界/角色/模拟过程

**核心趋势**: 2026年7月，中文 AI 社区出现**世界模拟引擎创业潮**——多个项目同时在做"LLM 驱动的涌现式世界"，从小说创作切入。技术路径相似：多智能体 + 长循环 + RAG + 世界状态推演。

**对数码宝贝世界的影响** ⭐⭐⭐⭐⭐:
- ⚠️ **赛道正在变热** — 7天内出现 3 个直接竞品
- 我们的差异化优势：**RPG 战斗系统 + 进化体系 + 数码宝贝 IP**（其他项目是通用世界模拟或小说引擎）
- 窗口期建议：加速完成 Phase 18-20，尽快展示"30只数码兽自主战斗/进化/社交"的完整 Demo
- **novel-studio 的技术路径值得关注**: Go 语言后端 + 本地 RAG + 多智能体长循环，性能可能优于 Python

#### 4. 【GitHub】LLM-Agents-Simulation-Framework (PRAISELab)
> ⭐11 | 基于 LLM 的生成式智能体社会动力学研究框架

**核心**: 学术级模拟框架，用于研究复杂社会动力学。

**对数码宝贝世界的影响** ⭐⭐⭐:
- 可作为评估基准——将我们的 30只数码兽 vs 他们的框架进行社会动力学指标对比
- 他们的"社会动力学"测量方法（极化度、影响力网络、信息传播速度）可引入我们的活力指标

#### 5. 【GitHub】Project_Psychohistory — 心理史学生成式 AI 历史学家
> ⭐8 | 受阿西莫夫《基地》启发，模拟文明的涌现历史

**核心**: 用 AI agent 扮演"生成式历史学家"，模拟文明从诞生到衰落的涌现历史。

**对数码宝贝世界的影响** ⭐⭐⭐:
- 我们的世界叙事系统（Phase 14）可以借鉴其"历史学家视角"——不只是生成当前事件，而是生成**事后回顾式的"历史记载"**
- Phase 18 的记忆健康诊断可以扩展为"文明健康诊断"——数码世界的"心理史学"指标
...
| 项目 | Stars | 定位 | 我们的优势 |
|------|-------|------|-----------|
| mirage-cortex | 152 | Agent Swarm 工具清单 | N/A — 这是索引，不是产品 |
| GoldenHouse-Writer | — | 中文 AI 小说世界模拟 | RPG战斗+进化+30agent规模 |
| novel-studio | 11 | Go 后端世界推演引擎 | 数码宝贝IP + 生产部署 |
| SoulSim | 5 | 通用世界模拟 | 战斗系统 + 进化树 + 关系追踪 |
| LLM-Agents-Sim-Framework | 11 | 学术社会动力学研究 | 游戏化 + 可玩性 |
| Project_Psychohistory | 8 | 文明涌现历史 | 实时交互 + 可视化 |

### 📊 赛道判断

**2026年7月 = "LLM 世界模拟"赛道启动月。** 多个项目同期出现，说明技术成熟度已达到临界点（LLM 成本下降 + Agent 框架成熟）。我们的窗口期约 **2-3 个月**——在竞品完善通用世界模拟之前，用**数码宝贝 IP + RPG 深度**建立护城河。

### 🎯 今日开发建议

1. **Phase 18 记忆健康诊断** — 推进，这是我们的独特功能（竞品无）
2. **Phase 17 收尾** — MBTI 人格雷达图上线后，确认前端显示正确
3. **README 更新** — 测试数更新至 779，Phase 17/18 加入路线图
4. **差异化强化** — 录制战斗/进化 Demo 视频，展示 RPG 深度

---

### 🔮 下次调研方向
- agent 长期记忆衰退与记忆健康的形式化定义
- DPO 人格注入在非对话场景（战斗、探索）中的应用
- 世界模拟引擎的评估基准（如何量化"涌现"质量）
